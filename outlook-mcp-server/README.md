...
| `ENABLE_WRITE_OPERATIONS`                                         | `true` to enable write tools (`send_email`, `create_draft`, `create_mail_folder`, `move_email`, `mark_as_read`, `create_reply_draft`, `set_message_categories`, `apply_llm_category_to_email`; adds `Mail.Send` and `Mail.ReadWrite` to default OAuth scopes when using OAuth)                                                                                                      |
