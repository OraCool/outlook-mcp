...
- **Read tools**: `get_email`, `get_thread` (by Graph `conversationId`; sorted by `receivedDateTime` in the server to avoid Graph **`InefficientFilter`** on `$filter`+`$orderby`), `search_emails` (KQL + optional read/date filters), `list_inbox` (optional folder, unread, received date filters), `list_folders`, `get_attachments`, `list_master_categories`
...
| `ENABLE_WRITE_OPERATIONS`                                         | `true` to enable write tools (`send_email`, `send_draft_email`, `create_draft`, `create_mail_folder`, `move_email`, `mark_as_read`, `create_reply_draft`, `set_message_categories`, `apply_llm_category_to_email`; adds `Mail.Send` and `Mail.ReadWrite` to default OAuth scopes when using OAuth)                                                                                                      |
...
| `PII_REDACTION_ENABLED`                                           | `true` to run **Microsoft Presidio** on email JSON **before** MCP sampling (`categorize_email`, `extract_email_data`); requires optional install `pip install ".[pii]"` and `python -m spacy download en_core_web_lg`                                                                                                                                                              |
...
| **Client secret** (Certificates & secrets → _Value_) | `GRAPH_OAUTH_CLIENT_SECRET` **or** `GRAPH_APPLICATION_CLIENT_SECRET`                          |
